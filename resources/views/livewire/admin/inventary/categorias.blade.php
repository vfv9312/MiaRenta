<div class="p-6 bg-gray-50 dark:bg-[#0f172a] min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <header class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Categorías</h1>
                <p class="mt-2 text-lg text-gray-600 dark:text-gray-400">Gestiona las categorías del inventario.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        @forelse ($categorias as $categoria)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-gray-900 dark:text-white">{{ $categoria->nombre }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 text-xs font-bold rounded-full {{ $categoria->status_id == 3 ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' }}">
                                        {{ $categoria->status->name ?? ($categoria->status_id == 3 ? 'Eliminado' : 'Activo') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $categoria->created_at->format('d/m/Y') }}</div>
                                </td>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center gap-3">
                                        <i class="fas fa-list text-4xl text-gray-300 dark:text-gray-600"></i>
                                        <p class="text-lg font-medium">No se encontraron categorías</p>
                                        <p class="text-sm">Comienza agregando una nueva.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div
                ...
                <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">
                        {{ $item_id ? 'Editar Categoría' : 'Nueva Categoría' }}
                    </h3>
                ...
                <form wire:submit.prevent="store" class="p-6">
                    <div class="space-y-5">
                        <!-- Nombre -->
                        <div>
                            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nombre de la
                                Categoría</label>
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar categoría?</h3>
                <p class="text-gray-500 dark:text-gray-400 mb-6 font-medium">Esta acción cambiará el estado de la
                    categoría a eliminada.</p>
